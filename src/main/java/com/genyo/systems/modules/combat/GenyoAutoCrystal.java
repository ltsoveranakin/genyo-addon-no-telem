...
public class GenyoAutoCrystal extends PlacerModule {
    ...
    public GenyoAutoCrystal() {
        super(Genyo.COMBAT, "Genyo AutoCrystal", "Automatically places and destroys crystals");
    }
    ...
    public void onDisconnect(DisconnectEvent event) {
        if (disableDeath.get()) {
            toggle();
            sendDisableMsg("Disabled on death.");
        } else {
            onDeactivate();
        }
    }
    ...
    public void onPlayerUpdate(PlayerTickEvent event) {
        if (mc.player.isSpectator() || isSilentSwap(autoSwap.get()) && Modules.get().get(GenyoAutoCity.class).isSilentSwapping()) {
            return;
        }
...
//        }
    ...
    public void placeCrystal(BlockPos blockPos, Hand hand, boolean checkPlacement) {
        ...
        // Vec3d vec3d3 = vec3d.add(vec3d1.x * placeRange.get(),
        //        vec3d1.y * placeRange.get(), vec3d1.z * placeRange.get());
    ...
    private void placeInternal(BlockHitResult result, Hand hand) {
        ...
        // Entity ID predict
        if (idPredict.get()) {
            boolean flag = Modules.get().isActive(GenyoAutoXP.class) || mc.player.isUsingItem() && mc.player.getStackInHand(mc.player.getActiveHand()).getItem() instanceof ExperienceBottleItem;
        ...
        for (Entity crystal : entities) {
            ...
            for (Entity entity : entities) {
                ...
                if (antiSurroundC.get() && entity instanceof PlayerEntity player
                    ...
                    BlockPos miningBlock = Modules.get().get(GenyoAutoCity.class).getMiningBlock();
                    if (Modules.get().isActive(GenyoAutoCity.class) && miningBlock != null) {
