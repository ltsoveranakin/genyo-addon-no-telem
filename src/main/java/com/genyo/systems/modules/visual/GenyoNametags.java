...
public class GenyoNametags extends GenyoModule {
    ...
    public void onActivate() {
        if (Modules.get().isActive(Nametags.class)) {
            Modules.get().get(Nametags.class).toggle();
            GenyoChatUtils.sendInfo("Deactivated Meteor's Nametags");
        }
    }
    ...
    private void renderNametagPlayer(Render2DEvent event, PlayerEntity player, boolean shadow) {
        ...
        int pops = 0;
        if (Managers.COMBAT.popList.get(player.getName().getString()) != null)
            pops = Managers.COMBAT.popList.get(player.getName().getString());

        String popsText = " -" + pops;

        if (pops < 3) popsColor = GREEN;
        else if (pops < 10) popsColor = AMBER;
        else popsColor = RED;
        ...
        if (displayPops.get() && pops > 0) width += popsWidth;
        ...
        if (displayPops.get() && pops != 0) hX = text.render(popsText, hX, hY, popsColor, shadow);
        ...
        if (displayItems.get()) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (stack.isDamageable() && itemDurability.get() != Nametags.Durability.None) {
